...
function AmbientBackdrop() {
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-0 overflow-hidden">
      ...
      {/* Floating gold particles — half as many on phones to keep things calm. */}
      {!reduced &&
...
function FeatureTile({ item, Icon, large = false }: { item: Item; Icon: React.ComponentType<{ className?: string }>; large?: boolean }) {
  return (
    <div className="group relative h-full overflow-hidden rounded-sm bg-[var(--color-ivory)]/85 p-6 ring-1 ring-[var(--color-line)] backdrop-blur-sm transition-shadow hover:shadow-[0_30px_60px_-30px_rgba(8,57,34,0.25)] sm:p-8">
      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--color-rlc-800)]/8 text-[var(--color-rlc-800)] transition group-hover:bg-[var(--color-gold)]/20 group-hover:text-[var(--color-gold)] sm:h-14 sm:w-14">
        <Icon className="h-5 w-5 sm:h-6 sm:w-6" />
      </div>
      <h3 className={`mt-6 font-[var(--font-display)] text-[var(--color-rlc-900)] sm:mt-8 ${large ? 'text-2xl sm:text-3xl' : 'text-lg sm:text-xl'}`}>{item.title}</h3>
      <p className="mt-3 text-sm text-[var(--color-ink-soft)] sm:text-base">{item.body}</p>
    </div>
  );
}
...
function ImageTile({ src, label, item }: { src: string; label: string; item: Item }) {
  return (
    <div className="group relative h-full min-h-[240px] overflow-hidden rounded-sm bg-[var(--color-rlc-900)] ring-1 ring-[var(--color-line)] sm:min-h-[260px]">
      ...
      <div className="absolute inset-x-6 bottom-6 text-[var(--color-cream)] sm:inset-x-8 sm:bottom-8">
        <div className="text-[0.7rem] uppercase tracking-[0.16em] opacity-70">{label}</div>
        <h3 className="mt-2 font-[var(--font-display)] text-xl sm:text-2xl">{item.title}</h3>
...
function AccentTile() {
  return (
    <div className="relative h-full overflow-hidden rounded-sm bg-[var(--color-rlc-900)] p-6 text-[var(--color-cream)] sm:p-8">
      <span aria-hidden className="absolute -end-10 -top-10 select-none font-[var(--font-display)] text-[10rem] leading-none text-[var(--color-gold)]/20 sm:text-[14rem]">✦</span>
      <div className="relative">
        <div className="font-[var(--font-display)] text-4xl text-[var(--color-gold)] sm:text-5xl">A+</div>
